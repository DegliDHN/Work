...
public class TrackedImageInfoManager : MonoBehaviour
{
	...
	private Coroutine hideTrackableCoro;
	...
	private IEnumerator HideImageIfCantTrackInSeconds(float timeInSeconds, ARTrackedImage trackedImage){
		Debug.Log("Start Hiding Coro");
		yield return new WaitForSeconds(timeInSeconds);
		if(trackedImage.trackingState != TrackingState.Tracking){
			trackedImage.gameObject.SetActive(false);
		}
		hideTrackableCoro = null;
		Debug.Log("Hide AR Content");
	}
	...
	protected virtual void InitTrackedObject(ARTrackedImage trackedImage)
	{
	}
}
