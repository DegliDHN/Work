...
using System.Linq;
	...
	public static class MonoBehaviourEx
	{
		...
		public static T GetComponent<T>(this MonoBehaviour mb, bool includeInactive) where T : Component
		{
			var comp = mb.GetComponentInChildren<T>(includeInactive);
			if (comp.gameObject == mb.gameObject)
			{
				return comp;
			}
			else
